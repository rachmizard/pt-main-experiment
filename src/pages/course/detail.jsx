      ...
      {loading ? (
        ...
        <>
          <Row className="justify-content-center gap-3">
            <Col lg={3}>
              <Image src={coursePic} className="w-100 h-100" />
            </Col>
            <Col lg={4}>
              <h2>{courseTitle}</h2>
              <InfoLabel icon={<FaMoneyBill />} text={`Rp. ${price}`} />
              <InfoLabel icon={<FaUsers />} text={`${totalUserJoin} members`} />
              <InfoLabel
                icon={<FaTags />}
                text={
                  tags?.length > 0 ? tags?.join(",") : "No tags are applied"
                }
              />
              <InfoLabel
                icon={<FaVideo />}
